...
int riot_picol_command_exec(struct picolInterp *i, int argc, char **argv, void *pd)
{
    int res = EXIT_SUCCESS;

    (void)i;

    shell_command_handler_t handler = find_handler(pd, argv[1]);
    if (handler != NULL) {
        if (IS_USED(MODULE_SHELL_HOOKS)) {
            shell_pre_command_hook(argc - 1, &argv[1]);
            res = handler(argc - 1, &argv[1]);
            shell_post_command_hook(res, argc - 1, &argv[1]);
        }
        else {
            res = handler(argc - 1, &argv[1]);
        }
